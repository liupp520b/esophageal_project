...
class Resnet(nn.Module):
    def __init__(self, dilate_scale=8, pretrained=True, backbone='resnet101'):
...
class PSPNet(nn.Module):
    def __init__(self, num_classes, downsample_factor, backbone="resnet101", pretrained=True, aux_branch=True):
        ...
        if backbone=="resnet50":
            self.backbone = Resnet(downsample_factor, pretrained, backbone)
            aux_channel = 1024
            out_channel = 2048
        if backbone=="resnet101":
            self.backbone = Resnet(downsample_factor, pretrained, backbone)
    ...
    def forward(self, x):
        ...
        if self.aux_branch:
            ...
            # output_aux = F.log_softmax(output_aux, dim=1)
...
# module = PSPNet(num_classes=21, downsample_factor=16, backbone="resnet101", pretrained = False, aux_branch=True)
# print(module)