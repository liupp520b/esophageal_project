...
class Encoder2D(nn.Module):
    def __init__(self, config: TransConfig, is_segmentation=True):
        super().__init__()
        self.config = config
        self.out_channels = config.out_channels  # 1
        self.bert_model = TransModel2d(config)
        sample_rate = config.sample_rate  # 4
        sample_v = int(math.pow(2, sample_rate))  # 16
        ...
        self.hh = self.patch_size[0] // sample_v  # 2
        self.ww = self.patch_size[1] // sample_v  # 2
    ...
    def forward(self, x):
        ...
        p1 = self.patch_size[0]  # 32
        p2 = self.patch_size[1]  # 32
...
class SETRModel(nn.Module):
    def __init__(self, patch_size=(32, 32),
                        ...
                        sample_rate=4,):
        super().__init__()
        config = TransConfig(patch_size=patch_size,   #（32, 32）
