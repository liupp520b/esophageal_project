...
class AxialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        N, W, C, H = x.shape  # (B, 32, 16, 32)
        x = x.contiguous().view(N * W, C, H)  # (B*32, 16, 32)

        # Transformations
        qkv = self.bn_qkv(self.qkv_transform(x)) # (N * W, C*2, H)(B*32, 16*2, 32)
        print("qkv:", qkv.size())
        q, k, v = torch.split(qkv.reshape(N * W, self.groups, self.group_planes * 2, H), [self.group_planes // 2, self.group_planes // 2, self.group_planes], dim=2)
        # Q (B*32, 8, 1, 32) K (B*32, 8, 1, 32) V (B*128, 8, 2, 32)
        # Calculate position embedding
        all_embeddings = torch.index_select(self.relative, 1, self.flatten_index).view(self.group_planes * 2, self.kernel_size, self.kernel_size) # (4, 16, 16)
        q_embedding, k_embedding, v_embedding = torch.split(all_embeddings, [self.group_planes // 2, self.group_planes // 2, self.group_planes], dim=0)
        # q_embedding (1, 16, 16) k_embedding (1, 16, 16) v_embedding (2, 16,16)
...
# print("x:", x.size())
# a = AxialAttention(in_planes=16, out_planes=16, groups=8, kernel_size=64)
# x = a(x)
...
class AxialAttention_wopos(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = self.bn_qkv(self.qkv_transform(x))
        q, k, v = torch.split(qkv.reshape(N * W, self.groups, self.group_planes * 2, H), [self.group_planes // 2, self.group_planes // 2, self.group_planes], dim=2)
        # Q (B*128, 8, 1, 128) K (B*128, 8, 1, 128) V (B*128, 8, 2, 128)
        qk = torch.einsum('bgci, bgcj->bgij', q, k) # (B*128, 8, 128, 128)
...
class AxialBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.conv_down(x)   # (B, 16, 32, 32)
        out = self.bn1(out)
        out = self.relu(out)
        # print(out.shape)
...
class ResAxialAttentionUNet(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        # pdb.set_trace()
        x = self.conv1(x)  # (B, 8, 128, 128)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.conv2(x)  # (B, 128, 128, 128)
        x = self.bn2(x)
        x = self.relu(x)
        x = self.conv3(x)   # (B, 8, 128, 128)
        x = self.bn3(x)
        x = self.relu(x)
...
class medt_net(nn.Module):
    ...
    def __init__(self, block, block_2, layers, num_classes=2, zero_init_residual=True,
        ...
        img_size_p = img_size // 4  # 32

        self.layer1_p = self._make_layer(block_2, int(128*s), layers[0], kernel_size= (img_size_p))  #  (AxialBlock_wopos, 16, 1, 32/2)
        ...
        self.layer3_p = self._make_layer(block_2, int(512*s), layers[2], stride=2, kernel_size=(img_size_p//2),
                                       dilate=replace_stride_with_dilation[1])
        self.layer4_p = self._make_layer(block_2, int(1024*s), layers[3], stride=2, kernel_size=(img_size_p//4),
                                       dilate=replace_stride_with_dilation[2])
    ...
    def _make_layer(self, block, planes, blocks, kernel_size=56, stride=1, dilate=False):  # (block=AxialBlock_wopos, planes=16, blocks=1,kernel_size=16)
        norm_layer = self._norm_layer
        downsample = None
        previous_dilation = self.dilation
        if dilate:
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, groups=self.groups,  # (self.inplanes=8, planes=16, stride=1,groups=8,base_width,base_width=64)
                            base_width=self.base_width, dilation=previous_dilation,
                            norm_layer=norm_layer, kernel_size=kernel_size))
        self.inplanes = planes * block.expansion
        if stride != 1:
    ...
    def _forward_impl(self, x):
        ...
        for i in range(0,4):
            for j in range(0,4):
                ...
                # begin patch wise
                x_p = self.conv1_p(x_p)  # (B,8,32,32)
                x_p = self.bn1_p(x_p)
                # x = F.max_pool2d(x,2,2)
                x_p = self.relu(x_p)

                x_p = self.conv2_p(x_p)   # (B,128,32,32)
                x_p = self.bn2_p(x_p)
                # x = F.max_pool2d(x,2,2)
                x_p = self.relu(x_p)
                x_p = self.conv3_p(x_p)  # (B,8,32,32)
                x_p = self.bn3_p(x_p)
                # x = F.max_pool2d(x,2,2)
                x_p = self.relu(x_p)
...
def MedT(pretrained=False, **kwargs):
    model = medt_net(AxialBlock_dynamic,AxialBlock_wopos, [1, 2, 4, 1], s= 1,  **kwargs)
    return model
