...
class CrissCrossAttention(nn.Module):
    ...
    def forward(self, x):
        m_batchsize, _, height, width = x.size()  # （B, C, H, W）
        proj_query = self.query_conv(x)   # （B, C, H, W）=（B, 64, 97, 97）
        ...
        proj_key = self.key_conv(x)  # （B, 64, H, W）
        ...
        proj_value = self.value_conv(x)   # (B, 512, H, W)
