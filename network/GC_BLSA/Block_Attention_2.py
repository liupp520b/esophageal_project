...
class Block_Attention_2(nn.Module):
    def __init__(self, in_dim, feature_size, patch_size):
        super(Block_Attention_2, self).__init__()
        ...
        # self.self_attention = self_attention(dim=in_dim // 8, v_dim=in_dim)
        # self.position_bias = nn.Parameter(torch.randn(16,16))
        self.relative = nn.Parameter(torch.randn(4,31))
        query_index = torch.arange(16).unsqueeze(0)
        key_index = torch.arange(16).unsqueeze(1)
        relative_index = key_index - query_index + 16 - 1
        self.register_buffer('flatten_index', relative_index.view(-1))
        self.affinity_bn = nn.BatchNorm2d(256)
    ...
    def forward(self, x):
        ...
        # print("q_loc:",q_loc.size(no_pos_with_cbm))
        ...
        Incidence_x = F.softmax(Incidence, dim=1)  # 块相关矩阵
        ...
        for i in range(0, 4):
            for j in range(0, 4):
                ...
                J_pos = q_embeding[:,stride*i:stride*(i+1),stride*j :stride*(j+1)]  # (b, 256, 4 ,4)
                # # print("J_pos", J_pos.size())
                K_pos = k_embeding[:,stride*i:stride*(i+1),stride*j :stride*(j+1)]  # (b, 256, 4 ,4)
                # # print("K_pos", K_pos.size())
                V_pos = v_embeding[:,stride*i:stride*(i+1),stride*j :stride*(j+1)]  # (b, 2048, 4 ,4)
                # print("V_pos", V_pos.size())
                J = J_loc[:,:,stride*i:stride*(i+1),stride*j :stride*(j+1)]
                # print("J", J.size())
                K = k_loc[:, :, stride * i:stride * (i + 1), stride * j:stride * (j + 1)]
                # print("K", K.size())
                V = v_loc[:, :, stride * i:stride * (i + 1), stride * j:stride * (j + 1)]
                # print("V", V.size())
                qr = torch.einsum('bgci,cij->bgij',J, J_pos)
                # # print('qr', qr.size())
                kr = torch.einsum('bgci,cij->bgij', K, K_pos)
                # print('kr', kr.size())
                qk = torch.einsum('bgci,bgcj->bgij',J, K).transpose(2,3)
                # print('qk', qk.size())
                affinity = qk + qr + kr
                affinity = F.softmax(self.affinity_bn(affinity), dim=1)
                # print("affinity",affinity.size())
                ar = torch.einsum('bgij,cij->bgij', affinity, V_pos)
                # print("ar",ar.size())
                av = torch.einsum('bgij,bgcj->bgci',affinity, V)

                output = self.output_bn(av + ar)
                x_loc[:, :, stride * i:stride * (i + 1), stride * j:stride * (j + 1)] = output
